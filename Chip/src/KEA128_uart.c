...
void uart_putbuff1 (UARTn uartn, int32 *buff, uint32 len)
{
    while(len--)
    {
        uart_putchar(uartn, *buff);
...
void uart_putstr1 (UARTn uartn, char *str)
{
    while(*str)
    {
        uart_putchar(uartn, *str++);
...
void uart_getchar (UARTn uartn, uint8 *ch) 
{
 ...
 uint8 uart_querychar (UARTn uartn, uint8 *ch)
  {
    if((uart[uartn]->S1 & UART_S1_RDRF_MASK))         //查询是否接受到数据
    {
      *ch =  uart[uartn]->D;      //接受八位数据
        return  1;                              //返回1表示接受成功
    }
